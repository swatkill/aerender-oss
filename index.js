...
const putOSS = async(region,accessKeyId,accessKeySecret,object,localFile,logger) =>{
    let client = new OSS({
        region: region,
...
const run = async (job, settings, action, type,logger) => {
    ...
    const {
        input,
        params: { region,accessKeyId,accessKeySecret,object },
    } = action;
    try {
        ...
        await putOSS(region,accessKeyId,accessKeySecret,object,finalInput, logger);
    } catch (error) {
        logger.log(
